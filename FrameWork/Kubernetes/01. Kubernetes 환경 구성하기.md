...
---
# Trouble Shooting
### "installations.operator.tigera.io" is invalid
calico를 설치하던 중 
metadata.annotations의 길이가 너무 길다는 에러가 발생했다.
calico관련 gitHub issue에서 해당 문제를 해결하는 방법이 나와있었다.
apply대신 create, replace를 사용하라는 안내였다.
...
1. Kernel 의 ip_filter 에서 충돌이 발생했다면 해당 파일의 아래 문구를 추가해주자.
...
2. "10.96.0.1:443: connect: no route to host가 발생했다면 방화벽 및 iptable을 재설정하자.
```bash
...
3. 공식문서를 확인해보니 새로운 버전이 업데이트되면서 `cidr`을 `192.168.0.0/16`로 사용하지 않는다면 calico 설치시 custom-resources.yaml을 다운 받아. 해당 부분을 수정해주어야 한다고 나와있다. `curl`을 사용하여 다운 받고 고쳐주도록 하자
```bash title:"custom-resources.yaml"
...
4. 다음과 같은 에러가 발생하고 안내 문구가 나왔다.
```bash
"Setting route_localnet=1 to allow node-ports on localhost;
to change this either disable iptables.localhostNodePorts
(--iptables-localhost-nodeports) or set nodePortAddresses
(--nodeport-addresses) to filter loopback addresses"
```

route_localnet은 Linux시스템에서 로컬 네트워크로 라우팅하는 것을 허용하는 커널 파라미터다. 아래 값을 변경해보자.
```bash
